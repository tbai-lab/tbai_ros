...
class SensorBridge {
   public:
    ...
    ~SensorBridge() {
        running_ = false;
        if (imgThread_.joinable()) imgThread_.join();
        if (pcThread_.joinable()) pcThread_.join();
    }
   ...
   private:
    ...
    std::thread imgThread_, pcThread_;
};
...
int main(int argc, char *argv[]) {
    ...
    controller.addController(std::make_unique<tbai::rl::RosBobController>(
        urdfString, anymalInterface, tbai::reference::getReferenceVelocityGeneratorShared(nh)));
