...
StateVisualizer::StateVisualizer(std::shared_ptr<tbai::RobotInterface> interface,
                                 const std::vector<std::string> &jointNames,
                                 const std::vector<std::string> &footFrameNames, double rate, bool enableContactVis,
                                 const std::string &baseName, const std::tuple<uint8_t, uint8_t, uint8_t> &contactColor)
...
void StateVisualizer::publishJointTF(const State &state, const ros::Time &stamp) {
...
void StateVisualizer::publishContacts(const State &state, const ros::Time &stamp) {
    visualization_msgs::MarkerArray markerArray;
    for (size_t i = 0; i < footFrameNames_.size() && i < state.contactFlags.size(); ++i) {
        visualization_msgs::Marker marker;
        marker.header.stamp = stamp;
        marker.header.frame_id = footFrameNames_[i];
        marker.id = i;
        ...
        marker.color.r = std::get<0>(contactColorNormalized_);
        marker.color.g = std::get<1>(contactColorNormalized_);
        marker.color.b = std::get<2>(contactColorNormalized_);
        marker.color.a = 1.0;
